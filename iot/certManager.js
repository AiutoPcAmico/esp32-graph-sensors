...
export function checkMasterCertificate() {
    return new Promise((resolve, reject) => {
        try {
            const file = fs.readFileSync(`./certs/${secretMasterName}.pem.crt`);
            resolve();
        } catch (err) {
            const finishedActions = [];

            const cert = fs.createWriteStream(`./certs/${secretMasterName}.certificate.pem.crt`);
            const privateKey = fs.createWriteStream(`./certs/${secretMasterName}.private.pem.key`);
            const amazonRoot = fs.createWriteStream(`./certs/AmazonRootCA1.pem`)
